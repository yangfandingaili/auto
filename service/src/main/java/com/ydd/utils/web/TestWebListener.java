 ...
 *
 */
public class TestWebListener  extends TestListenerAdapter{
	...
	public void onTestStart(ITestResult  tr)
	{
		//tr.getTestContext()
		//方法开始前初始化报表描述信息
		WebAssertion.errors.clear();
		WebAssertion.errorIndex=0;
		WebAssertion.messageList.clear();
	...
	public void onTestFailure( ITestResult  tr)
	{
		...
		TestWebBaseCase testBaseCase=(TestWebBaseCase) tr.getInstance();
		WebDriver driver=TestWebBaseCase.driver;
		ScreenShot screenShot=new ScreenShot(driver);
		//设置截图名字
		screenShot.setscreenName(tr.getMethod().getDescription()+WebAssertion.errorIndex.toString());
		log.error(WebAssertion.errorIndex.toString());
		screenShot.takeScreenshot();
		for(int i=0;i<WebAssertion.messageList.size();i++)
		{
...
//				sb.append("<err_assert_info"+"   method=\""+tr.getTestClass().getName()+"."+tr.getMethod().getMethodName()+"\">\n");
			...
			if(WebAssertion.messageList.get(i).contains("pass"))
			{
				sb.append("<span class=\"pass_span\">"+WebAssertion.messageList.get(i)+"</span></br>\n");
			}
			else if(WebAssertion.messageList.get(i).contains("failed"))
			{
				sb.append("<span class=\"err_span\">"+WebAssertion.messageList.get(i)+"</span></br>\n");
			}
			else if(WebAssertion.messageList.get(i).contains("点击查看大图"))
			{
				sb.append("<span class=\"err_span\">"+WebAssertion.messageList.get(i)+"</span>\n");
			}
...
//				sb.append("<err_assert_info_StackTrace"+"   method=\""+tr.getTestClass().getName()+"."+tr.getMethod().getMethodName()+"\">\n");
		...
		for (Error e : WebAssertion.errors) {
	...
	public void onTestSuccess(ITestResult tr) {
		for(int i=0;i<WebAssertion.messageList.size();i++)
		{
			...
			sb2.append("<span class=\"pass_span\">"+WebAssertion.messageList.get(i)+"</span></br>\n");
