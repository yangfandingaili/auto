...
import com.ydd.utils.FileManger;
 ...
 *
 */
public class TestMobileListener  extends TestListenerAdapter{
	...
	public void onTestStart(ITestResult  tr)
	{
		//tr.getTestContext()
		//方法开始前初始化报表描述信息
		MobileAssertion.errors.clear();
		MobileAssertion.errorIndex=0;
		MobileAssertion.messageList.clear();
		MobileElementAction.noSuchElementExceptions.clear();
	...
	public void onTestFailure( ITestResult  tr)
	{
		...
		TestMobileBaseCase testBaseCase=(TestMobileBaseCase) tr.getInstance();
		WebDriver driver=testBaseCase.driver;
		ScreenShot screenShot=new ScreenShot(driver);
		//设置截图名字
		screenShot.setscreenName(tr.getMethod().getDescription()+MobileAssertion.errorIndex.toString());
		log.error(MobileAssertion.errorIndex.toString());
		screenShot.takeScreenshot();
		for(int i=0;i<MobileAssertion.messageList.size();i++)
		{
...
//				sb.append("<err_assert_info"+"   method=\""+tr.getTestClass().getName()+"."+tr.getMethod().getMethodName()+"\">\n");
			...
			if(MobileAssertion.messageList.get(i).contains("pass"))
			{
				sb.append("<span class=\"pass_span\">"+MobileAssertion.messageList.get(i)+"</span></br>\n");
			}
			else if(MobileAssertion.messageList.get(i).contains("failed"))
			{
				sb.append("<span class=\"err_span\">"+MobileAssertion.messageList.get(i)+"</span></br>\n");
			}
			else if(MobileAssertion.messageList.get(i).contains("点击查看大图"))
			{
				sb.append("<span class=\"err_span\">"+MobileAssertion.messageList.get(i)+"</span>\n");
			}
			sb.append("</err_assert_info>\n");
		}


		for (Exception e : MobileElementAction.noSuchElementExceptions) {
...
//				sb.append("<err_assert_info_StackTrace"+"   method=\""+tr.getTestClass().getName()+"."+tr.getMethod().getMethodName()+"\">\n");
		...
		for (Error e : MobileAssertion.errors) {
	...
	public void onTestSuccess(ITestResult tr) {
		for(int i=0;i<MobileAssertion.messageList.size();i++)
		{
			...
			sb2.append("<span class=\"pass_span\">"+MobileAssertion.messageList.get(i)+"</span></br>\n");
